...
nest_species <- eggs_sum %>%
  left_join(nest_data, join_by(Nest_ID, Site, Year))
```
...
snowcover_data_fixed <- snowcover_data_fixed %>%
  mutate(Date = if_else(Date == "8&9 june 06", "08-June-06", Date))
...
snowcover_month <- snowcover_data_fixed %>% 
  mutate(snow_day = if_else(Snow_cover > 10, 1, 0)) %>%
  group_by(Site, Year, Month = month(Date2)) %>%
  summarise(cum_snow_days = sum(snow_day), avg_snow_cover = mean(Snow_cover, na.rm = TRUE))
